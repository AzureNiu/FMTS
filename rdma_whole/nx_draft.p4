	...
	inout ingress_intrinsic_metadata_for_tm_t ig_tm_md)
{
	...
	action send_and_copy(PortId_t port) {
	...
	action getSublen(bit<16> subLen, MirrorId_t sid, QueueId_t qid) {
		ig_md.mirror_hdr.subLen = subLen;
		ig_hdr.info.subLen = subLen;
		/* 设置镜像会话号 */
		ig_md.session_id = sid;
...
struct egress_header_t {
	...
	subpkg_4_h 		subpkg_4;
	subpkg_2_h 		subpkg_2;
	padding_2_h		padding_2;	// 用于子包4字节对齐
	subpkg_1_h 		subpkg_1;
	padding_1_h		padding_1;
	...
	out egress_intrinsic_metadata_t eg_intr_md)
{
	...
	/* 解析子包 */
	state parse_subpkg {
		transition select(eg_hdr.info.subLen[8:0]) {
			//1 0000 0000
	...
	state parse_subpkg_256 {
		pkt.extract(eg_hdr.subpkg_256);
		transition accept;
	}
	state parse_subpkg_128 {
		pkt.extract(eg_hdr.subpkg_128);
		transition accept;
	}
	state parse_subpkg_64 {
		pkt.extract(eg_hdr.subpkg_64);
		transition accept;
	}
	state parse_subpkg_32 {
		pkt.extract(eg_hdr.subpkg_32);
		transition accept;
	}
	state parse_subpkg_16 {
		pkt.extract(eg_hdr.subpkg_16);
		transition accept;
	}
	state parse_subpkg_8 {
		pkt.extract(eg_hdr.subpkg_8);
		transition accept;
	}
	state parse_subpkg_4 {
		pkt.extract(eg_hdr.subpkg_4);
		transition accept;
	}
	state parse_subpkg_2 {
		pkt.extract(eg_hdr.subpkg_2);
		//pkt.extract(eg_hdr.padding_2);
		transition accept;
	}
	state parse_subpkg_1 {
		pkt.extract(eg_hdr.subpkg_1);
		//pkt.extract(eg_hdr.padding_1);
		transition accept;
	}
}
	...
	inout egress_intrinsic_metadata_for_output_port_t eg_oport_md)
{
	/* 4字节对齐填充处理表 */
	action padding1() {
		eg_hdr.padding_1.setValid();
		eg_hdr.padding_1.buf = 0;
		eg_hdr.info.subLen = 4;
		eg_hdr.oldipv4.total_len = 21;
	}
	action padding2() {
		eg_hdr.padding_2.setValid();
		eg_hdr.padding_2.buf = 0;
		eg_hdr.info.subLen = 4;
		eg_hdr.oldipv4.total_len = 22;
	}
	action NoPadding() {
	...
	action delSubpkg() {
		eg_hdr.oldipv4.total_len = eg_hdr.oldipv4.total_len-eg_hdr.info.subLen;
		eg_hdr.subpkg_256.setInvalid();
		eg_hdr.subpkg_128.setInvalid();
		eg_hdr.subpkg_64.setInvalid();
		eg_hdr.subpkg_32.setInvalid();
		eg_hdr.subpkg_16.setInvalid();
		eg_hdr.subpkg_8.setInvalid();
		eg_hdr.subpkg_4.setInvalid();
		eg_hdr.subpkg_2.setInvalid();
		eg_hdr.subpkg_1.setInvalid();
	}
	...
	Register<pair_32, bit<1>>(1) outpkt_reg;
	RegisterAction<pair_32, bit<1>, bit<32>>(outpkt_reg) outpkt0_action = {
		void apply(inout pair_32 reg) {
			reg.left = reg.left + 1;
		}
	};
	action outpktCnt0() {
		outpkt0_action.execute(0);
	}
	RegisterAction<pair_32, bit<1>, bit<32>>(outpkt_reg) outpkt1_action = {
		void apply(inout pair_32 reg) {
			if (eg_hdr.oldipv4.total_len == 20) {
	...
	apply {
		...
		else if (eg_hdr.info.type == 1){
			/* 如果是原像，丢弃相应子包 */
			eg_md.tmp4[31:16] = 0;
			eg_md.tmp4[15:0] = eg_hdr.oldipv4.total_len+20;
			exszCnt0();
			exszCnt1();
			eg_md.tmp5[31:16] = 0;
