...
static int client_prepare_connection(struct sockaddr_in *s_addr) {
  ...
  get_addr("10.0.0.3", (struct sockaddr *)&client_sockaddr);
  client_sockaddr.sin_port = htons(32800);
  ret = rdma_resolve_addr(cm_client_id, (struct sockaddr *)&client_sockaddr, (struct sockaddr *)s_addr, 2000);*/
...
static int client_connect_to_server() {
  ...
  printf("src port: %d\n", ntohs(rdma_get_src_port(cm_client_id)));
  printf("dst port: %d\n", ntohs(rdma_get_dst_port(cm_client_id)));

  struct ibv_qp_attr qp_attr;
  ibv_query_qp(client_qp, &qp_attr, 0, &qp_init_attr);
...
static int client_xchange_metadata_with_server() {
  ...
  client_metadata_attr.address = 0;
  client_metadata_attr.length = len;
  client_metadata_attr.stag.local_stag = 0;
  ...
  fprintf(fp, "va_lo: %d\n", (uint32_t)(server_metadata_attr.address&0xffffffff));
  fprintf(fp, "va_hi_0: %d\n", (uint32_t)((server_metadata_attr.address>>32)&0xffffffff));
  uint32_t tmp = (uint32_t)((len>>32)/2);
  fprintf(fp, "va_hi_1: %d\n", (uint32_t)((server_metadata_attr.address>>32)&0xffffffff)+tmp);
...
void usage() {
  printf("Usage:\n");
  printf("rdma_client: [-a <server_addr>] [-p <server_port>] [-l <server_mr_len>]"
         "(required)\n");
  printf("(default IP is 127.0.0.1 and port is %d)\n", DEFAULT_RDMA_PORT);
  printf("(default length of MR at server is 16 ONE_GB)\n");
  exit(1);
}
...
int main(int argc, char **argv) {
  ...
  fp = fopen("/home/lab1806/niux/rdma_params.txt", "w");
  /* Parse Command Line Arguments */
  while ((option = getopt(argc, argv, "a:p:l:")) != -1) {
    switch (option) {
    case 'a':
    ...
    case 'p':
      ...
      break;
    case 'l':
      /* this overwrites the variable len */
      len_arg = strtoul(optarg, NULL, 0);
      len = len_arg*ONE_GB;
      break;
    default:
      usage();
  ...
  char cmd[64];
  while (1) {
    printf("You can input \"quit\" to quit.\n");
